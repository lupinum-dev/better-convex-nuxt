 ...
 *
 * Expected behavior:
 * - SSR: No fetch, renders with status=LoadingFirstPage, hasData=false
 * - Client nav: Instant, shows loading state
 */

const { results, status, isLoading, loadMore } = await useConvexPaginatedQuery(
  api.notes.listPaginated,
  {},
  { initialNumItems: 3, server: false, lazy: true },
)
...
<template>
  <div data-testid="server-false-lazy-true-page" class="test-page">
    <h1>Paginated: server: false, lazy: true</h1>
    <p class="description">No SSR fetch, instant client navigation with loading state</p>
