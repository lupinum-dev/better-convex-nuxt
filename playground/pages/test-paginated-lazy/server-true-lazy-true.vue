 ...
 *
 * Expected behavior:
 * - SSR: Fetches data, renders with status != LoadingFirstPage, hasData=true
 * - Client nav: Instant (non-blocking), shows loading state initially
 * - Uses await so SSR waits for data
 */

const { results, status, isLoading, loadMore } = await useConvexPaginatedQuery(
  api.notes.listPaginated,
  {},
  { initialNumItems: 3, server: true, lazy: true },
)
...
<template>
  <div data-testid="server-true-lazy-true-page" class="test-page">
    <h1>Paginated: server: true, lazy: true</h1>
    <p class="description">SSR fetches data, instant client navigation (best of both worlds)</p>
