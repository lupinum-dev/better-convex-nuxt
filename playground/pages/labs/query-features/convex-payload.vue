...
const bytesLength = computed(() => data.value?.b instanceof ArrayBuffer ? data.value.b.byteLength : -1)
...
<template>
  <div data-testid="convex-payload-page" class="test-page">
    ...
    <div class="row"><span>status:</span> <span data-testid="status">{{ status }}</span></div>
    <div class="row"><span>pending:</span> <span data-testid="pending">{{ pending }}</span></div>
    <div class="row"><span>error:</span> <span data-testid="error">{{ error?.message ?? 'null' }}</span></div>
    <div class="row"><span>bigint type:</span> <span data-testid="bigint-type">{{ bigintType }}</span></div>
    <div class="row"><span>bigint value:</span> <span data-testid="bigint-value">{{ bigintValue }}</span></div>
    <div class="row"><span>bytes type:</span> <span data-testid="bytes-type">{{ bytesType }}</span></div>
    <div class="row"><span>bytes length:</span> <span data-testid="bytes-length">{{ bytesLength }}</span></div>
    <div class="row"><span>bytes values:</span> <span data-testid="bytes-values">{{ bytesValues }}</span></div>
    <div class="row">
