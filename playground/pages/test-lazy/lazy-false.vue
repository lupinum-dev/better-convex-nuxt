...
/**
 * Test page for lazy: false (default) behavior
 *
 * Expected behavior:
 * - Navigation blocks until data loads
 * - initial-pending = false
 * - initial-has-data = true
 * - Data is immediately available when page renders
 */

const { data, pending, status } = useConvexQuery(api.notes.list, {}, {
  verbose: true,
})
// Note: lazy defaults to false, so we don't specify it
...
<template>
  <div data-testid="lazy-false-page" class="test-page">
    <h1>lazy: false (Single Query, Default)</h1>
