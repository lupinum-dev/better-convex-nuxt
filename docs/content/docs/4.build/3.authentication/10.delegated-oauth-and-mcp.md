...
| `pnpm test:mcp-auth`              | Two direct preregistered public-client PKCE flows plus live MCP authorization |
| `pnpm test:mcp-conformance`       | Locked-RC official-client proof plus published 2025 protocol scenarios        |
...
`test:mcp-conformance` uses the official beta.5 client for the locked
`2026-07-28` request envelope. The older official conformance package is retained
only for the published `2025-11-25` initialize, ping, and tools-list scenarios
because it advertises no RC scenarios. Neither path certifies the OAuth
