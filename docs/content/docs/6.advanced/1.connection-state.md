...
| `hasEverConnected`    | `ComputedRef<boolean>`           | Whether the client has connected at least once   |
| `hasInflightRequests` | `ComputedRef<boolean>`           | Whether any requests are currently in flight     |
| `connectionRetries`   | `ComputedRef<number>`            | Current reconnect attempt counter                |
...
| `isHydratingConnection` | `Ref<boolean>`                 | Initial hydration grace-window flag              |
| `pendingMutations`    | `ComputedRef<number>`            | Pending mutations count                          |
| `pendingActions`      | `ComputedRef<number>`            | Pending actions count                            |
