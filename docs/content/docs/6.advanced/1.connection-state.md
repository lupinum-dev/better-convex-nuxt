...
const { isConnected, isReconnecting, connectionRetries, shouldShowOfflineUi } = useConvexConnectionState()
</script>

<template>
  <div v-if="isReconnecting && shouldShowOfflineUi" class="offline-banner">
...
`useConvexConnectionState()` now includes `isHydratingConnection` and `shouldShowOfflineUi` to prevent the common "offline banner flash" during the first ~500ms of client hydration.
::
...
| `isHydratingConnection` | `Readonly<Ref<boolean>>`       | Initial client connection grace window |
| `shouldShowOfflineUi` | `ComputedRef<boolean>`           | Offline UI flag with hydration flash suppression |
...
```vue
<script setup lang="ts">
const { shouldShowOfflineUi, hasEverConnected } = useConvexConnectionState()

// Only show banner if we were connected before
const showOfflineBanner = computed(() =>
  hasEverConnected.value && shouldShowOfflineUi.value
)
</script>
...
- [Fetching Data](/docs/data-fetching/queries) - Queries with real-time updates
- [SSR & Hydration](/docs/server-side/ssr-hydration) - Handling loading states
