...
2. **Use `skipAuthRoutes`** for public pages. Saves one API request per page load by skipping the auth token exchange.
3. **Enable `authCache`** for high-traffic authenticated pages. Caches Convex JWT tokens server-side to reduce TTFB by 50-200ms on subsequent requests.
...
::warning
Enabling `authCache` introduces a revocation window. If a Better Auth session is revoked server-side, SSR can still render as authenticated until cache TTL expires.
::
