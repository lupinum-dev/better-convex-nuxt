...
Use the `clearAuthCache` server utility to immediately invalidate cached tokens when a user logs out. This is auto-imported from `#imports` in server routes.
...
`clearAuthCache(sessionToken: string): Promise<void>` - Removes the cached Convex JWT for the given session token from the Nitro storage layer (memory or Redis). Without this call, the cached token remains valid until TTL expires.
