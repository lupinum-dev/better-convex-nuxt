...
3. **Enable `auth.cache`** for high-traffic authenticated pages. Caches Convex JWT tokens server-side to reduce TTFB by 50-200ms on subsequent requests.
...
::warning
Enabling `auth.cache` introduces a revocation window. If a Better Auth session is revoked server-side, SSR can still render as authenticated until cache TTL expires.
::
...
Use the `serverConvexClearAuthCache` server utility to immediately invalidate cached tokens when a user logs out. This is auto-imported from `#imports` in server routes.
...
export default defineEventHandler(async (event) => {
  ...
  if (sessionToken) {
    // Removes this user's cached Convex JWT from the Nitro storage layer
...
`serverConvexClearAuthCache(sessionToken: string): Promise<void>` - Removes the cached Convex JWT for the given session token from the Nitro storage layer (memory or Redis). Without this call, the cached token remains valid until TTL expires.

### Configuration Options

`auth.cache` is a `false | { ttl?: number }` value: omitted or `false` disables caching; an object enables it.

| Option | Type     | Default | Description                                     |
| ------ | -------- | ------- | ----------------------------------------------- |
| `ttl`  | `number` | `60`    | Cache TTL in seconds (1 min max, clamped to 60) |
...
| Session revocation | Immediate             | Up to TTL delay (use `serverConvexClearAuthCache` for immediate) |
