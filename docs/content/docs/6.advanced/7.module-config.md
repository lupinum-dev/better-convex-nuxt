...
convex: {
  defaults: {
    ...
    lazy: false,     // Block navigation until data loads (default)
...
| Option      | Type               | Default                        | Description |
| ----------- | ------------------ | ------------------------------ | ----------- |
| `authCache` | `AuthCacheOptions` | `{ enabled: false, ttl: 900 }` | SSR auth token caching. See [Performance](/docs/advanced/performance#ssr-auth-token-caching). |
