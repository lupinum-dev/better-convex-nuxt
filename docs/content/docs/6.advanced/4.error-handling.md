...
const { pending, error, reset } = createPost
...
async function handleSubmit() {
  ...
  try {
    await createPost({ title: title.value })
    title.value = ''
    navigateTo('/posts')
  } catch (e) {
...
### Choosing Callable vs `safe()`

Call the mutation directly by default. Use `safe()` when you explicitly need a non-throwing boundary.

| API              | Best for                                        | Error style                                                   |
| ---------------- | ----------------------------------------------- | ------------------------------------------------------------- |
| `mutation(args)` | Standard app/domain flows                       | Throws on transport/runtime errors; use `try/catch` if needed |
| `safe(args)`     | Middleware/startup/background/legacy boundaries | Never throws; always returns `CallResult<T>`                  |

For UI flows where you want to avoid `try/catch` boilerplate, use `safe()`:

```ts
const createPost = useConvexMutation(api.posts.create)
const { error } = createPost

const result = await createPost.safe({ title: 'Hello' })
...
If your backend endpoint already returns an app-level `CallResult<T>`, then `safe()` wraps that result:

```ts
// Endpoint return type: CallResult<User>
const fromCallable = await mutation(args) // CallResult<User>

const fromSafe = await mutation.safe(args) // CallResult<CallResult<User>>
```

This is expected. `safe()` handles thrown/runtime failures; your inner `CallResult` handles domain/business outcomes.
...
Use `kind`, `code`, and `status` for transport-level decisions. Define a stable application error shape in Convex and inspect `data` for product decisions. Never classify errors by matching human-readable messages.
...
export default defineNuxtPlugin(() => {
  ...
  window.addEventListener('unhandledrejection', (event) => {
    ...
    if (error instanceof ConvexCallError && error.status === 401) {
...
const isNetworkError = computed(() => error.value?.kind === 'transport')
</script>

<template>
  <div v-if="status === 'error'">
    <p v-if="isNetworkError">Could not reach Convex. Please check your connection.</p>
...
```vue
<script setup lang="ts">
const deletePost = useConvexMutation(api.posts.delete)
const { error } = deletePost

type ProductErrorData = { code: 'FORBIDDEN' | 'POST_NOT_FOUND' }

const isForbidden = computed(
  () => (error.value?.data as ProductErrorData | undefined)?.code === 'FORBIDDEN',
)
</script>
