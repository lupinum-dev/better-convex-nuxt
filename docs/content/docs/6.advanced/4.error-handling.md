...
async function handleSubmit() {
  ...
  try {
    ...
    // Error is also available in createPost.error.value
...
<template>
  <form @submit.prevent="handleSubmit">
    <input v-model="title" :disabled="createPost.pending.value" @input="createPost.reset()" />

    <p v-if="formError" class="text-red-500">
      {{ formError }}
    </p>

    <button :disabled="createPost.pending.value">
...
```vue
<script setup lang="ts">
const { isConnected, isReconnecting, pendingMutations } = useConvexConnectionState()
</script>
...
<template>
  ...
  <div v-if="pendingMutations > 0" class="pending-badge">Saving...</div>
</template>
```
...
[better-convex-nuxt] ▷ query ✗ api.posts.list 45ms
  └─ error: ConvexCallError "Unauthorized", retriable
```
...
const isForbidden = computed(() => deletePost.error.value?.message?.includes('Forbidden'))
</script>

<template>
  <div v-if="deletePost.error.value">
    <p v-if="isForbidden">You don't have permission to delete this post.</p>
    <p v-else>{{ deletePost.error.value.message }}</p>
  </div>
</template>
```
...
const validationError = computed(() => {
  if (createPost.error.value?.message?.startsWith('Validation:')) {
    return createPost.error.value.message.replace('Validation: ', '')
  }
  return null
})
</script>
