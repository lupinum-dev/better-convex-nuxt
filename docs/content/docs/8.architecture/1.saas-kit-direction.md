...
- Reword `createPermissions()` docs as UI capability helpers.
...
- Reword existing permissions docs so `createPermissions()` is a UI capability helper, not the SaaS authorization layer.
