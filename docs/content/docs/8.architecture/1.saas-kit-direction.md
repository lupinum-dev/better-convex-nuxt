...
- Better Auth owns users, sessions, and the data for each explicitly enabled and verified auth plugin. Organization, API-key, admin, passwordless, and passkey data may remain Better Auth-owned when the matching plugin has current schema and executable evidence; ownership does not imply that every plugin is supported by this module. Follow the [authentication security support boundary](/docs/auth-security/authentication#security-support-boundary-for-auth-plugins).
...
- Nuxt typed client plugin DX through `defineConvexAuthClient()` + `useConvexAuth().client`.
...
### Research Candidates, Not Release Support

Earlier isolated research exercised parts of these surfaces. This list is not a supported-plugin claim: every candidate still needs matching-version server/client/schema evidence and its complete HTTP, cookie, authorization, failure, and packed-runtime ceremony before it can become an opt-in recipe.
...
- Email OTP, magic-link, and passkey candidates.
- Generic OAuth with a synthetic provider.
- OAuth bearer-token product-route candidates with explicit recipe logic.
...
- Better Auth two-factor authentication is unsupported on Better Auth 1.6.23 with Convex Better Auth 0.12.5: the packaged schema omits lockout fields and the adapter cannot atomically increment failed attempts. A future supported order must place `twoFactor()` before `convex()`.
- The standalone Better Auth OIDC Provider and MCP plugins are unsupported through the fixed Nuxt proxy because their prompt cookies are outside the supported Better Auth namespace. The narrower OIDC discovery/JWKS behavior embedded by `convex()` is unaffected.
- Email OTP, magic link, passkey, generic OAuth, Admin, Organization, and API Key research does not substitute for a current executable compatibility matrix. Enable only the combinations explicitly covered by the security support boundary and deployment evidence.
...
- `useConvexAuth()`.
- `useConvexUser()`.
- `defineConvexAuthClient()` + typed `useConvexAuth().client`.
...
Do not copy a second auth factory from this research document. The maintained
`starters/agentic-saas/convex/auth.ts` file is the executable organization/access-control
pattern, and the [authentication guide](/docs/guide/auth) owns the fail-closed secret,
static-origin, password, plugin-order, and route-registration contract. Any future SaaS
template must extend that one pattern and keep `requireEmailVerificationOnInvitation: true`;
it must not create a parallel factory in architecture prose.
...
Declare the organization plugin once in the auth-client definition; `useConvexAuth().client` is then typed with it.
...
export function useSaasAuthClient() {
  ...
  if (!client) throw new Error('Auth client is unavailable during SSR')
  return client
}
```
