...
```vue
<script setup lang="ts">
// Redirect if user doesn't have permission after render
usePermissionRedirect({
...
Redirects after render when the current user lacks a permission. Enforce access in Convex functions.
