...
::callout{icon="i-lucide-info"}
**Route middleware + Convex queries:** If you check permissions inside `defineNuxtRouteMiddleware()`, use `useConvexQuery(..., { subscribe: false })`. This is strict HTTP-only mode (no WebSocket dependency), which fits route middleware's one-shot guard behavior.
::
...
`serverConvexQuery()` from `better-convex-nuxt/server` is still the right choice for **server-only** code (API routes, Nitro server middleware, webhooks), but it does not run during client-side route navigation.
...
**Tip**: You can wrap the base composables to add custom helpers like `isOwner`, `isAdmin`, or app-specific context. See the [Extending with Custom Helpers](/docs/auth-security/permissions-setup#extending-with-custom-helpers) section in the setup guide.
::
