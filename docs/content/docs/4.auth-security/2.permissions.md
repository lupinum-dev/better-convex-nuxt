...
`fetchQuery()` from `better-convex-nuxt/server` is still the right choice for **server-only** code (API routes, Nitro server middleware, webhooks), but it does not run during client-side route navigation.
