...
::callout{icon="i-lucide-info"}
**Route middleware + Convex queries:** If you check permissions inside `defineNuxtRouteMiddleware()`, use `useConvexQuery(..., { subscribe: false })`. Route middleware is a one-shot guard (not a component), so real-time subscriptions are unnecessary there.
::
