...
| ---------- | ------- | ----------- |
| `useAuth()` | Full auth API (state + actions) | Default choice. Sign in, sign up, sign out, read user state. |
| `useConvexAuth()` | Lower-level reactive state | Programmatic checks, route middleware, advanced use cases. |
| `useAuthClient()` / custom client | Raw Better Auth client | Plugin-specific APIs (`admin`, `organization`, etc.). |

`useAuth()` extends `useConvexAuth()` with `signIn`, `signUp`, and `client` - so everything on `useConvexAuth()` is also available on `useAuth()`.
...
Use `useAuth()` for both reactive auth state and standard auth actions.
...
- `client` is available on `useAuth()` as an advanced escape hatch for plugin-specific Better Auth APIs
::

::callout{icon="i-lucide-lightbulb"}
**What should I use?**
- Show user name/avatar: `useAuth().user`
- Sign in / sign up: `useAuth().signIn`, `useAuth().signUp`
- Sign out: `useAuth().signOut`
- Better Auth plugin methods (`admin`, `organization`, ...): `useAuth().client` or a custom typed client
::

### Using Additional Better Auth Plugins

`useAuth().client` returns the module-managed Better Auth client used by `better-convex-nuxt`.
...
If you need to query Convex in route middleware (for role/permission checks), use `useConvexQuery(..., { subscribe: false })` (strict HTTP-only mode, no WebSocket dependency).
...
<script setup lang="ts">
const { signIn, isAuthenticated, refreshAuth } = useAuth();
...
| `signOut`         | `() => Promise<T \| null>`          | Signs out from both Better Auth and Convex               |
| `refreshAuth`     | `() => Promise<void>`               | Force refresh Convex auth state (fetches fresh token)    |

### useAuth Returns

`useAuth()` extends `useConvexAuth()` with the following additional properties:
