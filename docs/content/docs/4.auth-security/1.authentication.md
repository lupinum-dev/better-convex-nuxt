...
- `convex/auth.ts` as the user-edited Better Auth file
- `convex/authBridge.ts` as the generated Convex plumbing
...
::note
`convex/auth.ts` stays raw Better Auth. The generated `convex/authBridge.ts` owns the Convex adapter wiring, sync triggers, trusted origins defaults, and the idempotent `createUserIfNeeded` bootstrap mutation.
::
