---
title: 'Authentication'
description: 'Set up authentication with Better Auth and Convex.'
---

## Overview

`@lupinum/trellis` uses [Better Auth](https://www.better-auth.com/) running on [Convex](https://www.convex.dev/) for authentication. Better Auth handles user registration, sessions, and OAuth providers, while Convex stores the user data and issues JWT tokens for authenticated queries and mutations.

The module provides:

- **SSR-safe auth state** -- the server plugin reads the session cookie, exchanges it for a JWT, and hydrates auth state to the client with zero flash of unauthenticated content.
- **Reactive composables** -- `useConvexAuth()` exposes auth state, the Better Auth client, sign-out, and manual refresh; `useConvexAuthActions()` wraps sign-in and sign-up flows with refresh, error handling, and redirect support.
- **Auth components** -- `<ConvexAuthenticated>`, `<ConvexUnauthenticated>`, `<ConvexAuthLoading>`, and `<ConvexAuthError>` for conditional rendering.
- **Route protection** -- protect pages with `definePageMeta({ convexAuth: true })`.
- **Automatic token management** -- the client reuses recent tokens briefly and fetches a fresh JWT when the current one is missing or too close to expiry.

```mermaid
sequenceDiagram
    participant Browser
    participant NuxtServer as Nuxt Server
    participant AuthProxy as Auth Proxy (/api/auth)
    participant Convex as Convex Backend

    Browser->>NuxtServer: Page request (with session cookie)
    NuxtServer->>AuthProxy: Exchange cookie for JWT
    AuthProxy->>Convex: Forward to /api/auth/convex/token
    Convex-->>AuthProxy: JWT token
    AuthProxy-->>NuxtServer: JWT token
    NuxtServer->>Convex: Query with JWT (HTTP)
    Convex-->>NuxtServer: Query results
    NuxtServer-->>Browser: HTML with data + hydrated auth state
    Browser->>Convex: WebSocket connection (with JWT)
    Convex-->>Browser: Real-time updates
```

## Setup

::steps

### Install the peer dependency

Better Auth support on Convex is provided by `@convex-dev/better-auth`. Install it alongside your existing `convex` and `@lupinum/trellis` packages:

::code-group

```bash [pnpm]
pnpm add @convex-dev/better-auth
```

```bash [yarn]
yarn add @convex-dev/better-auth
```

```bash [npm]
npm install @convex-dev/better-auth
```

```bash [bun]
bun add @convex-dev/better-auth
```

::

### Scaffold the auth files

```bash
npx trellis init auth
```

This writes:

- `convex/auth.ts` with `defineAuth(...)` and your Better Auth setup
- `convex/auth.config.ts`, `convex/http.ts`, and `convex/convex.config.ts` as generated support files

### Create `convex/auth.config.ts`

Convex needs an auth provider config so it knows how to verify the JWTs Better Auth issues for Convex:

```ts [convex/auth.config.ts]
import type { AuthConfig } from 'convex/server'
import { getAuthConfigProvider } from '@convex-dev/better-auth'

export default {
  providers: [getAuthConfigProvider()],
} satisfies AuthConfig
```

::tip
`getAuthConfigProvider()` sets up a `customJwt` provider with issuer `CONVEX_SITE_URL`, `applicationID: 'convex'`, algorithm `RS256`, and the default JWKS URL `${CONVEX_SITE_URL}/api/auth/convex/jwks`.
::

### Create `convex/auth.ts`

Configure Better Auth with `defineAuth(...)`. This keeps the Convex component client, sync triggers,
and Better Auth plugin wiring in one file:

```ts [convex/auth.ts]
import { betterAuth } from 'better-auth'
import { defineAuth } from '@lupinum/trellis/auth'

import { components, internal } from './_generated/api'
import { mutation } from './_generated/server'
import authConfig from './auth.config'

export const { authComponent, createAuth, createUserIfNeeded } = defineAuth(
  { components, internal, mutation, authConfig },
  {
    emailPassword: true,
    custom: (_ctx, bridge) =>
      betterAuth({
        baseURL: bridge.siteUrl,
        database: bridge.database,
        emailAndPassword: {
          enabled: true,
        },
        socialProviders: {
          github: {
            clientId: process.env.GITHUB_CLIENT_ID!,
            clientSecret: process.env.GITHUB_CLIENT_SECRET!,
          },
        },
        plugins: [bridge.createConvexPlugin()],
        trustedOrigins: bridge.trustedOrigins,
      }),
  },
)

export const { onCreate, onUpdate, onDelete } = authComponent.triggersApi()
```

::note
`convex/auth.ts` now owns both your Better Auth options and the Convex adapter setup. The generated
scaffold is a starting point, not a second bridge file.
::

### Create `convex/http.ts`

Register the Better Auth HTTP routes so Convex can handle authentication requests:

```ts [convex/http.ts]
import { httpRouter } from 'convex/server'
import { authComponent, createAuth } from './auth'

const http = httpRouter()

authComponent.registerRoutes(http, createAuth)

export default http
```

### Set environment variables in the Convex dashboard

Navigate to your [Convex dashboard](https://dashboard.convex.dev/) and add the following environment variables to your deployment:

::field-group
::field{name="BETTER_AUTH_SECRET" type="required"}
A random secret string used to sign session tokens. Generate one with `openssl rand -hex 32`.
::
::field{name="SITE_URL" type="required"}
Your app's origin, e.g. `http://localhost:3000` in development or `https://myapp.com` in production. Better Auth uses this for callback URLs, redirect validation, and trusted origin checks.
::
::

::warning
`BETTER_AUTH_SECRET` must be kept secret. Never commit it to version control. Set it exclusively in the Convex dashboard under **Settings > Environment Variables**.
::

### Set environment variables locally

Your `.env.local` file should contain the Convex deployment URL. This is typically already present if you have run `npx convex dev`:

```bash [.env.local]
CONVEX_URL=https://your-deployment-123.convex.cloud
```

### Enable auth in your Nuxt config

Enable authentication in your `nuxt.config.ts`:

```ts [nuxt.config.ts]
export default defineNuxtConfig({
  modules: ['@lupinum/trellis'],

  trellis: {
    auth: true,
  },
})
```

Setting `auth: true` enables the full authentication flow with all defaults. You can also pass an object for fine-grained control:

```ts [nuxt.config.ts]
export default defineNuxtConfig({
  modules: ['@lupinum/trellis'],

  trellis: {
    auth: {
      routeProtection: {
        redirectTo: '/login',
        preserveReturnTo: true,
      },
    },
    },
  },
})
```

::

::note{title="About the auth proxy"}
When `auth` is enabled, the module creates a catch-all server route at `/api/auth/[...]` that proxies authentication requests to your Convex site URL. This is necessary because session cookies must be set on the same domain as your application -- since Convex runs on `*.convex.site`, the browser would block cookies set directly by Convex. The proxy keeps cookies on your app's domain while forwarding requests to Convex.

The proxy handles forwarding auth requests, preserving `Set-Cookie` headers, forwarding the host/proto/client-IP headers Better Auth expects, CORS validation, and OAuth redirect flows. The route path defaults to `/api/auth` and can be customized with `auth.route` in your config.
::

::tip
If you want the full explanation of why Convex needs `/api/auth/convex/jwks`, what signs the JWT, and when static `JWKS` changes the setup, read [JWKS & Convex JWTs](/docs/auth-security/jwks-and-convex-jwts).
::

## Auth config options

The `auth` option accepts either `true` for defaults or an object with these properties:

::field-group
::field{name="enabled" type="boolean" default="true"}
Enable or disable auth. When using the object form, defaults to `true`.
::
::field{name="route" type="string" default="'/api/auth'"}
Custom route path for the auth proxy endpoint.
::
::field{name="routeProtection" type="object"}
Configure route protection behavior. See [Route Protection](/docs/auth-security/route-protection).
::
::field{name="routeProtection.redirectTo" type="string" default="'/auth/signin'"}
Where to redirect unauthenticated users on protected routes.
::
::field{name="routeProtection.preserveReturnTo" type="boolean" default="true"}
Append `?redirect=/original-path` to the redirect URL so users return after sign-in.
::
::field{name="unauthorized" type="object"}
Configure behavior when Convex returns a 401/403 for authenticated queries or mutations.
::
::field{name="cache" type="object"}
SSR auth token caching. See [Token Refresh](/docs/auth-security/token-refresh).
::
::field{name="trustedOrigins" type="string[]" default="[]"}
Additional trusted origins for CORS on the auth proxy. Same-origin is always allowed. Supports wildcards like `https://preview-*.vercel.app`.
::
::field{name="skipAuthRoutes" type="string[]" default="[]"}
Routes that skip auth token fetches. Supports glob patterns like `/docs/**`.
::
::field{name="proxy" type="object"}
Body size limits for the auth proxy (`maxRequestBodyBytes`, `maxResponseBodyBytes`).
::
::
