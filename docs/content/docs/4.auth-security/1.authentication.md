...
Only cache routes that never hydrate a token. `skipAuthRoutes` skips client auth bootstrapping on matching paths; it does **not** prevent SSR token hydration if the server plugin resolves a session for that request. SSR responses that carry a token send `Cache-Control: private, no-store`, but platform-level ISR, route-rule caching, or CDN rules can ignore handler headers. Do not enable shared caching for authenticated routes.
...
```vue
<script setup lang="ts">
const { signIn, signOut, isAuthenticated, user, refreshAuth } = useConvexAuth()
...
If you need plugin-specific client methods like `authClient.admin.listUsers()`, create a typed plugin client with `createBetterConvexAuthClient()`.
The helper prepends `convexClient()`, uses the configured Nuxt auth proxy route, and keeps `credentials: 'include'` by default.
...
#### Client: create a typed plugin client
...
::callout{icon="i-lucide-alert-triangle"}
**Common mistake:** calling `authClient.signOut()` directly in app code and expecting `useConvexAuth().isAuthenticated` to update after logout.
...
- Treat a Convex query (for example `usePermissions()` / `api.auth.getPermissionContext`) as the server-read path for Better Auth roles and org membership.
