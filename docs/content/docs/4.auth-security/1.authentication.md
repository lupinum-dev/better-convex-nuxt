...
### `useAuth().client` (Advanced Escape Hatch)

| Return       | Type                 | Description                 |
| ------------ | -------------------- | --------------------------- |
| `authClient` | `AuthClient \| null` | Better Auth client instance |

`useAuth().client` returns the module-managed base client. For plugin-specific typed methods (for example `adminClient()` APIs or `inferAdditionalFields(...)` typing), create a custom client composable as shown above.
