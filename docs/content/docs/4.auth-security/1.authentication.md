...
| ---------- | ------- | ----------- |
| `useConvexAuth()` | Full auth API (state + actions) | Default choice. Sign in, sign up, sign out, read user state. |
...
`useConvexAuth()` extends `useConvexAuth()` with `signIn`, `signUp`, and `client` - so everything on `useConvexAuth()` is also available on `useConvexAuth()`.
...
::callout{icon="i-lucide-info"}
Use `serverConvexQuery()` only in **server-only** contexts (API routes, Nitro `server/middleware`, webhooks). Nuxt route middleware also runs during client-side navigation, so `serverConvexQuery()` alone will not cover SPA navigation.
::
...
`useConvexAuth()` extends `useConvexAuth()` with the following additional properties:
