...
Scope caching to routes that never hydrate a token: public marketing/docs pages via [`skipAuthRoutes`](#skipping-auth-checks), or pages where `auth.enabled` is `false`.
