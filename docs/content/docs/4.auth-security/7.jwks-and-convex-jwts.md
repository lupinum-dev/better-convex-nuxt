...
Trellis uses this mode automatically for the local example launcher after local Convex boots. That avoids the local-only Better Auth warning caused by Convex verifying JWTs through an internal JWKS self-request that has no client IP.
...
It does not. It reads auth state, exposes the Better Auth `client`, and provides `signOut()` plus `refreshAuth()`.

For email/password or email sign-up flows, pair `useConvexAuth().client` with `useConvexAuthActions()`. For redirect-based OAuth flows, call `client.signIn.social(...)` directly.
