...
const { usePermissions: useBasePermissions, usePermissionRedirect: basePermissionRedirect } =
  createPermissions<Permission, AppPermissionContext, Resource>({
    query: api.auth.getPermissionContext,
    checkPermission,
  })
...
export function usePermissions() {
  ...
  const canManageOrg = computed(() => base.role.value === 'owner' || base.role.value === 'admin')

  // App-specific context from the permission query
  const orgName = computed(() => base.user.value?.orgName ?? null)
...
// Re-export redirect helper with custom login path
...
This is a UX redirect helper. Keep permission enforcement in your Convex queries and mutations.

```vue [pages/settings.vue]
<script setup lang="ts">
import { usePermissionRedirect } from '~/composables/usePermissions'

// Redirect to /dashboard if user can't access settings after render
