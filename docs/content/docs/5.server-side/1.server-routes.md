...
If you need Convex data inside `defineNuxtRouteMiddleware()` (for auth/permissions during navigation), use `useConvexQuery(..., { subscribe: false })`. This is a strict HTTP-only mode (no WebSocket dependency), which matches route middleware's one-shot nature. Route middleware runs for both SSR and client-side navigation, while `serverConvexQuery()` is server-only.
...
export default defineEventHandler(async (event) => {
  ...
  const posts = await serverConvexQuery(
...
export default defineEventHandler(async (event) => {
  ...
  // Store in Convex
  const imported = await serverConvexMutation(
    config.public.convex.url,
...
export default defineEventHandler(async (event) => {
  ...
  try {
    const result = await serverConvexMutation(
      config.public.convex.url,
      api.tasks.create,
      { text: 'New task' }
    )
