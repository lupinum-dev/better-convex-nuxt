...
If you need Convex data inside `defineNuxtRouteMiddleware()` (for auth/permissions during navigation), use `useConvexQuery(..., { subscribe: false })`. Route middleware runs for both SSR and client-side navigation, while `fetchQuery()` is server-only.
