...
There is no per-route auth-skip config. On marketing/public pages, use `auth: 'none'` on the
queries that don't need identity — they execute immediately without waiting on auth settlement:

```ts
const { data } = useConvexQuery(api.pages.getMarketingContent, {}, { auth: 'none' })
```
...
<template>
  <ClientOnly>
    ...
    <!-- Empty -->
    <div v-else-if="status === 'success' && !posts?.length">No posts yet.</div>
...
<template>
  ...
  <!-- Idle: query skipped -->
  <div v-if="status === 'idle'" class="hint">Select a category to see posts</div>
...
<template>
  <ClientOnly>
    <!-- idle: query skipped -->
    <div v-if="status === 'idle'" class="idle-state">[Prompt to enable query]</div>
...
```vue
<script setup lang="ts">
const { data: org, status } = await useConvexQuery(
...
<template>
  <ClientOnly>
    ...
    <div v-else-if="postsStatus === 'success'">
      <PostCard v-for="post in posts" :key="post._id" :post="post" :categories="categories" />
    </div>
    <template #fallback>
      <SkeletonPostCard v-for="i in 3" :key="i" />
    </template>
...
const { data: dashboard, status } = await useConvexQuery(
  api.dashboard.get,
...
<template>
  <div class="page">
    <!-- Auth loading -->
    <div v-if="isPending" class="loading">Checking authentication...</div>
