...
See [Skipping Auth Checks](/docs/auth-security/authentication#skipping-auth-checks) for details.
...
const {
  ...
  computed(() => (category.value ? { category: category.value } : undefined)),
)
</script>
...
<template>
  ...
  <!-- Idle: query skipped -->
  <div v-if="status === 'skipped'" class="hint">Select a category to see posts</div>
...
<template>
  <ClientOnly>
    <!-- idle: query skipped -->
    <div v-if="status === 'skipped'" class="idle-state">[Prompt to enable query]</div>
