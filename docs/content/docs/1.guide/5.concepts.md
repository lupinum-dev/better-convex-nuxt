...
| `server: true, lazy: false`  | Fetches data, blocks render     | Blocks navigation until data loads      | **Default**. Critical content that must be visible immediately. Same as Nuxt's `useFetch` default. |
...
```ts
const { data, pending } = useConvexQuery(
  api.session.get,
  {},
  { server: false, lazy: true },
);
...
- **Want fast client navigation?** Add `lazy: true`.
