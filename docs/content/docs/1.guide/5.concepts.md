...
Choose loading mode with API shape (`useConvexQuery` vs `useConvexQueryLazy`) plus `server`:

| Pattern | SSR Behavior | Client Navigation | Use Case |
| ------- | ------------ | ----------------- | -------- |
| `await useConvexQuery(..., { server: false })` | No fetch, renders pending state | Blocks navigation until data loads | Client-only data that should not be in SSR HTML (for example session data). |
| `useConvexQueryLazy(..., { server: false })` | No fetch, renders pending state | Instant navigation, shows loading state | Non-critical, client-only content. |
| `await useConvexQuery(...)` | Fetches data, blocks render | Blocks navigation until data loads | **Default**. Critical content visible immediately. |
| `useConvexQueryLazy(..., { server: true })` | Fetches data, blocks render | Instant navigation, shows loading state | SSR on first visit, fast client navigation. |

### Choosing the Right Strategy

**`await useConvexQuery(..., { server: true })`** (Default)
...
- Same as Nuxt's `useFetch` default behavior
...
**`useConvexQueryLazy(..., { server: false })`**

```ts
const { data, pending } = useConvexQueryLazy(
  api.session.get,
  {},
  { server: false },
);
...
**`useConvexQueryLazy(..., { server: true })`** (Recommended for SEO content)
...
- **Want fast client navigation?** Use `useConvexQueryLazy`.
