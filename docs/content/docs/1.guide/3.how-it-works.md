---
title: 'How It Works'
description: 'Understand the architecture behind @lupinum/trellis: SSR, hydration, and real-time subscriptions.'
---

`@lupinum/trellis` bridges the gap between Nuxt's server-rendered architecture and Convex's real-time WebSocket protocol. This page explains the key architectural decisions and how data flows through the system.
...
```mermaid
flowchart TD
    Config[nuxt.config.ts] --> Module[Trellis module]
...
On the backend, every `app.query()` and `app.mutation()` runs through a fixed pipeline before your handler body executes. Understanding this flow helps you read any handler at a glance:

```mermaid
flowchart TD
    A[Request arrives] --> B[Actor resolution]
    B --> C{Guard check}
    C -->|open| D[Actor may be null]
    C -->|named guard| E[Actor narrowed to non-null]
    D --> F{Load phase}
    E --> F
    F -->|load defined| G[Fetch resource]
    F -->|no load| H[Skip]
    G --> I{Authorize check}
    H --> J[Handler body]
    I -->|passes| J
    I -->|fails| K[Forbidden error]
```

Each phase has a distinct responsibility:

::field-group
::field{name="Actor resolution" type="createApp options"}
Runs the `actor` resolver you passed to `createApp`. Returns your actor type (user ID, tenant ID, role, etc.) or `null` if unauthenticated. Configured once per app, inherited by all handlers.
::
::field{name="Guard" type="guard: open | defineGuard(...)"}
The entry gate. Determines whether the actor is allowed to call this function at all. If the guard is not `open`, the actor type is narrowed to `NonNullable` in all subsequent phases — a type error if the actor could be null.
::
::field{name="Load" type="load?: (ctx, args) => loaded"}
Optional. Fetches the resource the handler will operate on. Runs after the guard, so you know the actor is authorized to reach this point. The loaded value is passed to both `authorize` and `handler`.
::
::field{name="Authorize" type="authorize?: { check, label? }"}
Optional. Resource-level permission check. Runs after `load`, so it can inspect the loaded resource. Use this for rules like "only the owner can update this todo" that depend on data you had to fetch first.
::
::field{name="Handler" type="handler: (ctx, args, loaded) => result"}
Your business logic. By the time it runs, the actor is resolved, the entry guard passed, the resource is loaded, and the resource-level authorization is confirmed.
::
::

::tip
When reading a handler, the signature tells the story: `guard` says who can enter, `load` says what gets fetched, `authorize` says what resource-level rule applies, and `handler` is just business logic. One hop to the guard definition tells you the actual predicate.
::
