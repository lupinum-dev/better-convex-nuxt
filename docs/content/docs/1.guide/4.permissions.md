...
::tip
This guide keeps the full step-by-step setup. Use [Permissions Setup](/docs/auth-security/permissions-setup) and [Permissions Reference](/docs/auth-security/permissions) when you want the canonical deep-dive/reference pages too.
::
...
const { execute: createTask } = useConvexMutation(api.tasks.create)
...
const { execute: switchRole } = useConvexMutation(api.tasks.switchRole)
