---
title: Getting Started
description: Learn how to integrate Convex into your Nuxt 4 application.
navigation:
  icon: i-lucide-rocket
---

::tip{icon="i-lucide-target"}
**What you'll build:** A real-time task list that renders on the server (SSR) and syncs live across all connected clients via WebSocket.
::

::note
**Prerequisite:** You need a Convex account to follow this tutorial. If you don't have one, you can [sign up for free here](https://auth.convex.dev/sign-up).
::

### Create a new Nuxt project

First, initialize a fresh Nuxt project using the minimal template.

::code-group

```bash [pnpm]
pnpm create nuxt@latest my-convex-app
```

```bash [bun]
bun create nuxt@latest my-convex-app
```

```bash [npm]
npm create nuxt@latest my-convex-app
```

::

### Install dependencies

Install the Convex client and the Nuxt module.

::code-group

```bash [pnpm]
pnpm add convex better-convex-nuxt
```

```bash [bun]
bun add convex better-convex-nuxt
```

```bash [npm]
npm add convex better-convex-nuxt
```

::

### Configure Nuxt

Add the module to your `nuxt.config.ts` and provide the Convex deployment URL.

```ts {3, 5-7} [nuxt.config.ts]
export default defineNuxtConfig({
  compatibilityDate: '2025-07-15',
  modules: ['better-convex-nuxt'],

  convex: {
    url: process.env.CONVEX_URL,
  },
})
```

### Initialize Convex

Run the Convex dev command. This will prompt you to log in and create a new project. It will also generate a `.env.local` file containing your `CONVEX_URL`.

```bash
npx convex dev
```

::tip
Keep this terminal window running! It automatically syncs your backend functions whenever you save changes in the `convex/` folder.
::

### Update Environment Handling

By default, Convex saves variables to `.env.local`, but Nuxt looks for `.env`. Update your local Nuxt scripts to include the correct flag consistently.

```json {3-5} [package.json]
{
  "scripts": {
    "dev": "nuxt dev --dotenv .env.local",
    "build": "nuxt build --dotenv .env.local",
    "typecheck": "nuxt typecheck --dotenv .env.local"
  }
}
```

::tip
If you want deterministic localhost Convex URLs instead of a hosted dev deployment, see
[Local Development](/docs/deployment/local-development). The module itself does not require a new
config flag for this; you can drive it entirely from `USE_LOCAL_CONVEX` inside `nuxt.config.ts`.
::

### Prepare Sample Data

Create a file to define some initial data for your database.

```json [sampleData.jsonl]
{"text": "Buy groceries", "isCompleted": true}
{"text": "Go for a swim", "isCompleted": true}
{"text": "Integrate Convex", "isCompleted": false}
```

Now, import this data into a table named `tasks`:

```bash
npx convex import --table tasks sampleData.jsonl
```

### Define the Schema

Create a schema file to describe your data structure. This ensures type safety across your full stack.

```ts [convex/schema.ts]
import { defineSchema, defineTable } from 'convex/server'
import { v } from 'convex/values'

export default defineSchema({
  tasks: defineTable({
    text: v.string(),
    isCompleted: v.boolean(),
  }),
})
```

### Create a Database Query

Define a function to fetch the tasks from the database.

```ts [convex/tasks.ts]
import { query } from './_generated/server'

export const get = query({
  args: {},
  handler: async (ctx) => {
    return await ctx.db.query('tasks').collect()
  },
})
```

### Display Data in your App

Use the `useConvexQuery` composable to subscribe to your data. It fetches on the server (SSR) and updates in real-time via WebSocket.

```vue [app/app.vue]
<script setup lang="ts">
import { api } from '~~/convex/_generated/api'

// Subscribes to the 'get' function in 'convex/tasks.ts'
// Data is fetched on the server (SSR enabled by default)
const { data: tasks, error } = await useConvexQuery(api.tasks.get, {})
</script>

<template>
  <main>
    <h1>My Tasks</h1>

    <div v-if="error">Error: {{ error.message }}</div>

    <ul v-else>
      <li v-for="task in tasks" :key="task._id">
        {{ task.isCompleted ? '✅' : '❌' }} {{ task.text }}
      </li>
    </ul>
  </main>
</template>
```

::tip{icon="i-lucide-zap"}
**SSR by default:** Like Nuxt's `useFetch`, queries run on the server during SSR. Your page renders with data already loaded - no loading spinners on initial page load!
::

### Start Development

Launch your app and view it at `http://localhost:3000`.

::code-group

```bash [pnpm]
pnpm dev
```

```bash [bun]
bun dev
```

```bash [npm]
npm run dev
```

::

## Project Structure

After following the steps above, your project should look like this:

::code-tree{defaultValue="convex/tasks.ts"}

```ts [convex/schema.ts]
import { defineSchema, defineTable } from 'convex/server'
import { v } from 'convex/values'

export default defineSchema({
  tasks: defineTable({
    text: v.string(),
    isCompleted: v.boolean(),
  }),
})
```

```ts [convex/tasks.ts]
import { query } from './_generated/server'

export const get = query({
  args: {},
  handler: async (ctx) => {
    return await ctx.db.query('tasks').collect()
  },
})
```

```vue [app/app.vue]
<script setup lang="ts">
import { api } from '~~/convex/_generated/api'

const { data, error } = await useConvexQuery(api.tasks.get, {})
</script>

<template>
  <div>
    <p v-if="error">Error: {{ error.message }}</p>

    <ul v-else>
      <li v-for="task in data" :key="task._id">
        {{ task.text }}
      </li>
    </ul>
  </div>
</template>
```

```ts [nuxt.config.ts]
export default defineNuxtConfig({
  modules: ['better-convex-nuxt'],
  convex: { url: process.env.CONVEX_URL },
})
```

```env [.env.local]
CONVEX_DEPLOYMENT=dev:***-***-***
CONVEX_URL=https://***-***-***.convex.cloud
```

::note
`CONVEX_DEPLOYMENT` is written by the Convex CLI and is useful metadata for Convex tooling, but `better-convex-nuxt` itself reads `CONVEX_URL`.
::

::

## Troubleshooting

::accordion
::accordion-item{label="Why is my data not loading?" icon="i-lucide-circle-help"}
Ensure that `npx convex dev` is running in a separate terminal. This syncs your local functions to the Convex cloud.
::
::accordion-item{label="Environment Variable issues?" icon="i-lucide-circle-help"}
Check your `.env.local` file. It should contain `CONVEX_URL`. Make sure you added the `--dotenv .env.local` flag to your package.json scripts.
::
::

For the full production env ownership table, including `CONVEX_SITE_URL`, `NUXT_PUBLIC_CONVEX_URL`, and auth-related variables, see [Environment Matrix](/docs/deployment/environment-matrix).

## Next Steps

You've successfully set up a real-time Nuxt application with Convex! Continue learning with these guides:

::card-group

## ::card

title: Basics
icon: i-lucide-book-open
to: /docs/guide/basics

---

Learn about SSR, mutations, and controlling data loading behavior.
::

## ::card

title: Authentication
icon: i-lucide-shield-check
to: /docs/guide/auth

---

Add user authentication with Better Auth and protect your data.
::

## ::card

title: Query Options
icon: i-lucide-database
to: /docs/data-fetching/queries

---

Explore all `useConvexQuery` options: transforms, defaults, and more.
::

::
