...
From this point on, every handler built with `app.query(...)` or `app.mutation(...)` has access to the resolved actor. Guards operate on actors, not principals — so the same permission check covers every caller type.
...
export const publishPage = app.mutation({
...
| **authorize** | *(optional)* Resource-level check — can this actor touch *this specific* record? |
| **preview** | *(optional)* Confirmation summary for destructive actions |
...
- **Use `projectTool(...)` for Convex-backed tools.** Reserve `defineTool(...)` for tools that are Nitro-native (DNS lookups, external API calls).
