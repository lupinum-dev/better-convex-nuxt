...
If you need Better Auth plugins (for example `admin`) or typed Better Auth `additionalFields` on the frontend, use `createBetterConvexAuthClient()` in addition to `useConvexAuth().client` (or instead of it).
...
### Frontend plugin client (`app/composables/useExtendedAuthClient.ts`)
