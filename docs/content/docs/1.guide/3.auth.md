...
export default defineNuxtConfig({
  ...
  convex: {
    url: process.env.CONVEX_URL,
    // auth is enabled by default; set `auth: false` to disable it
  },
})
```
...
**That's it!** The module registers the same-origin `/api/auth/*` Nitro endpoint for both SSR and client navigation. Auth-enabled deployments must provide that server endpoint; a fully static host without Nitro is not a supported auth topology.
...
::note
The auth proxy makes exactly one request to the configured Convex site origin. It never follows redirects server-side; OAuth/login redirects are returned to the browser.
::
...
async function handleSignIn() {
  ...
  try {
    ...
    if (authError) {
      ...
      // signIn/signUp synchronize Convex auth automatically
      router.push('/')
    }
  } finally {
    loading.value = false
...
::note
The auth proxy route (`/api/auth/*`) is automatically created by the module whenever auth is enabled (the default). You don't need to create any server routes manually.
::
...
export default defineNuxtConfig({
  ...
  convex: {
    url: process.env.CONVEX_URL,
    // auth is enabled by default (set `auth: false` to disable); siteUrl auto-derived
  },
})
```
