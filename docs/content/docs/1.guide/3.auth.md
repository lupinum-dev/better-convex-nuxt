...
- Enables auth composables (`useAuth`, `useConvexAuth`)
...
If you need Better Auth plugins (for example `admin`) or typed Better Auth `additionalFields` on the frontend, use a **custom Better Auth client composable** in addition to `useAuth().client` (or instead of it).
