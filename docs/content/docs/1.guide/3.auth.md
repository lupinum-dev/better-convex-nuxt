...
::tip
This guide stays explicit on purpose for first-time setup. When you need deeper behavior details or deployment ownership, use [Authentication Reference](/docs/auth-security/authentication) and [Environment Matrix](/docs/deployment/environment-matrix).
::
...
For production, keep the ownership split clear:

- Nuxt/app host env: `CONVEX_URL` and optionally `CONVEX_SITE_URL`
- Convex Dashboard env: `SITE_URL`, `BETTER_AUTH_SECRET`, and provider credentials

`SITE_URL` is your app origin. It is not the same as `CONVEX_SITE_URL`.
...
For the complete deployment-oriented variable matrix, see [Environment Matrix](/docs/deployment/environment-matrix).
