...
If your app uses `ssr: true` (the default), you need a server proxy to forward auth requests and preserve cookies. See [SSR & Hydration](/docs/server-side/ssr-hydration) for more details on SSR patterns.
