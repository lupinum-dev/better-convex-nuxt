...
You can declare `useConvexAction()` during SSR or normal component setup. Only `execute()` requires a live client and must run where the Convex client is available.
::
...
const { execute: sendEmail, pending, status, error } = useConvexAction(api.emails.send)
...
```ts
const { execute } = useConvexAction(api.reports.generate, {
...
```vue
<script setup lang="ts">
const { execute: generateImage, pending, data: imageUrl } = useConvexAction(api.ai.generateImage)
...
```vue
<script setup lang="ts">
const { execute: processFile, pending, status, data } = useConvexAction(api.files.process)
...
<template>
  ...
  <div v-if="status === 'success'">Processed! Result: {{ data }}</div>
</template>
```
...
const {
  ...
  pending,
  error,
} = useConvexAction(api.payments.createCheckoutSession)
...
```vue
<script setup lang="ts">
const { execute, status, data, error, reset } = useConvexAction(api.reports.generate)
</script>

<template>
  <div class="status-card">
    <template v-if="status === 'idle'">
      <button @click="execute({ month: 'january' })">Generate Report</button>
    </template>
...
const { execute: saveOrder } = useConvexMutation(api.orders.create)
