...
const { execute: addTodo, pending: isAdding } = useConvexMutation(
  api.todos.create,
  {
    optimisticUpdate: (localStore, args) => {
      updateQuery({
...
const { execute: toggleTodo } = useConvexMutation(
  api.todos.toggle,
  {
    optimisticUpdate: (localStore, args) => {
      updateQuery({
...
const { execute: deleteTodo } = useConvexMutation(
  api.todos.remove,
  {
    optimisticUpdate: (localStore, args) => {
      deleteFromQuery({
...
const { results: messages, status, loadMore } = await useConvexPaginatedQuery(
  api.messages.list,
...
const { execute: sendMessage, pending } = useConvexMutation(
  api.messages.send,
  {
    optimisticUpdate: (localStore, args) => {
      insertAtTop({
...
const { execute: deleteMessage } = useConvexMutation(
  api.messages.delete,
  {
    optimisticUpdate: (localStore, args) => {
      deleteFromPaginatedQuery({
...
<template>
  <div class="chat">
    <div class="messages">
      ...
      <button v-if="status === 'ready'" @click="loadMore(50)">
        Load older messages
      </button>
    </div>
