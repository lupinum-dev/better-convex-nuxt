...
// Update a list query
ctx
  .query(api.notes.list, { userId: '123' })
  .update((notes) => (notes ? [...notes, newNote] : [newNote]))
...
ctx
  .paginatedQuery(api.notes.listPaginated, { userId: '123' })
  .insertAtTop({ ...args, _id: 'temp', _creationTime: Date.now() })

// Delete from paginated list
ctx.paginatedQuery(api.notes.listPaginated, { userId: '123' }).deleteItem(args.id)
...
ctx.matchQuery(api.notes.list).update((current, args) => {
  ...
  return current.map((note) => (note._id === updatedId ? { ...note, title: newTitle } : note))
})
```
...
<template>
  <button @click="deleteTask({ id: task._id })" :disabled="deleteTask.pending.value">Delete</button>
</template>
```
