...
The Convex CLI connects or creates a deployment, writes its selection and
public URLs to `.env.local`, creates `convex/`, and generates
`convex/_generated/api`.

Keep that first command running while developing backend functions. On later
runs, select only from the file it created:
...
Keep `.env.local` as the single ignored local configuration file. Nuxt defaults
...
If TypeScript reports that `convex/_generated/api` is missing, run
