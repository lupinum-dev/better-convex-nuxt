...
├── .env.example                  # committed names with secret values blank
├── .env.local                    # ignored local deployment selection and URLs
├── nuxt.config.ts                # module and runtime behavior
└── package.json
```

`better-convex-nuxt-convex configure` creates the local deployment selection in
`.env.local`.
Later Convex commands select through that exact file. Run Nuxt
commands with `--dotenv .env.local` so both tools consume that same file. Do not
keep a sibling `.env` with duplicated deployment values. Production values are
configured directly in the hosting environment.
