...
description: 'Add confirmation flows to destructive MCP tools using Convex-side preview and tool.'
---
...
- **`previewOf(...)`** wraps the operation's preview as a standalone query. Register it with `query(...)` so the MCP bridge can call it.
