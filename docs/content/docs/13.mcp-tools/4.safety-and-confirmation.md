...
- the MCP execute step must present the `_confirmationToken` returned by preview
- preview remains query-lane, so preview observability should be read as an outer request/runtime signal; execute is the semantic destructive boundary
