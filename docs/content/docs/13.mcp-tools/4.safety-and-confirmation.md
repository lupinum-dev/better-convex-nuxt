...
sequenceDiagram
    ...
    MCP->>Convex: Re-run preview, verify token, execute mutation
    Convex-->>MCP: Success
```
...
That last case is the important one: Trellis re-runs the preview before execution.
If the semantic destructive state drifted, the tool must preview again.
