...
const pageQuery = useConvexPaginatedQueryLazy(api.posts.byCategory, filters, { initialNumItems: 10 })
```
...
| `unauthenticated`          | `boolean`                | `false` | Skip auth token checks (for unauthenticated queries) |
...
Use `useConvexPaginatedQueryLazy` with `server: true` for SSR-rendered data and instant client navigation:
...
const { results } = useConvexPaginatedQueryLazy(api.messages.list, {})
// results is ComputedRef<Message[]>

// Args are typed
const { results } = useConvexPaginatedQueryLazy(
