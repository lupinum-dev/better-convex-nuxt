...
| `args`    | `MaybeRefOrGetter<Args \| 'skip'>` | Query arguments; `'skip'` disables |
...
| `initialData`      | `Item[] \| () => Item[]` | -       | Initial placeholder rows while loading        |
| `transform`        | `(items: Item[]) => T[]` | -       | Transform concatenated results                |
| `keepPreviousData` | `boolean`                | `false` | Keep last settled rows during arg transitions |

If `transform` is provided, `initialData` output is transformed too.
...
| `isStale`     | `ComputedRef<boolean>`              | Previous rows are shown for older args |
...
## Skipping and Stale Rows

Pass `'skip'` as args when a paginated query should not run:

```ts
const channelId = ref<string | null>(null)

const messages = await useConvexPaginatedQuery(
  api.messages.byChannel,
  () => (channelId.value ? { channelId: channelId.value } : 'skip'),
  { initialNumItems: 20 },
)
```

Use `keepPreviousData` when changing args should keep the previous rows visible while the first page for the new args loads. `isStale` is true while those rows belong to older args:
