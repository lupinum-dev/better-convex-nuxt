...
When args are reactive, pass `ref()`/`computed()` wrappers. Passing a plain `reactive()` object directly will not trigger re-fetches.

```ts
// WRONG: plain reactive object
const filters = reactive({ category: 'all' })
const pageQuery = useConvexPaginatedQuery(api.posts.byCategory, filters, { initialNumItems: 10 })

// RIGHT: computed/ref wrapper
const safeArgs = computed(() => ({ category: filters.category }))
const pageQuery2 = useConvexPaginatedQuery(api.posts.byCategory, safeArgs, { initialNumItems: 10 })
```
