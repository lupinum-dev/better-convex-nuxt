...
`useConvexPaginatedQueryLazy` is the synchronous counterpart — it returns refs immediately without blocking `<script setup>`. Use it in **layouts**, **reusable composables**, or anywhere `await` would lose the Vue component instance context.
...
const { results, status, loadMore, isLoading } = useConvexPaginatedQueryLazy(
  api.messages.list,
  {},
  { initialNumItems: 10 },
)
...
`useConvexPaginatedQueryLazy` accepts the same arguments and options and returns the same shape as `useConvexPaginatedQuery`.

::callout{icon="i-lucide-lightbulb"}
**When to use which:**
- **`useConvexPaginatedQuery`** — pages and components where you want navigation to wait for the first page (SSR-friendly, no loading flash).
- **`useConvexPaginatedQueryLazy`** — layouts, shared composables, or any context where `await` is problematic. The first page arrives asynchronously; handle the `loading-first-page` status in your template.
::
