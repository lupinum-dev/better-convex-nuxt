...
Use the `lazy: true` option for the synchronous counterpart — it returns refs immediately without blocking `<script setup>`. Use it in **layouts**, **reusable composables**, or anywhere `await` would lose the Vue component instance context.
...
<template>
  ...
  <div v-else>
    <div v-for="message in results" :key="message._id">{{ message.body }}</div>
    <button v-if="status === 'ready'" @click="loadMore(10)" :disabled="isLoading">Load More</button>
  </div>
</template>
```

The `lazy: true` option returns the same shape as the awaited version of `useConvexPaginatedQuery`.

::callout{icon="i-lucide-lightbulb"}
**When to use which:**
- **`useConvexPaginatedQuery`** (awaited) — pages and components where you want navigation to wait for the first page (SSR-friendly, no loading flash).
- **`useConvexPaginatedQuery(..., { lazy: true })`** — layouts, shared composables, or any context where `await` is problematic. The first page arrives asynchronously; handle the `loading-first-page` status in your template.
::
...
Use the fluent builder API for optimistic updates with paginated queries:
...
See [Optimistic Updates](/docs/mutations/optimistic-updates) for the full fluent builder API.
::
