...
const { data: post, pending } = useConvexQueryLazy(
  api.posts.getBySlug,
...
5. `useConvexQueryLazy` allows fresh detail data to load in the background.
...
- Use `useConvexQueryLazy` on detail pages to avoid navigation blocking.
