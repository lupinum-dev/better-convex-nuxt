...
- `useConvexQuery(...)`
- `initialData` for immediate placeholder/cached values
- `useNuxtData(...)` to read existing `useAsyncData` cache entries
- `getQueryKey(...)` to compute the exact cache key used by `useConvexQuery`

`getQueryKey` is auto-imported by the Nuxt module, like the query and mutation composables.
...
const { data: post, pending } = await useConvexQuery(
  ...
  {
    initialData: () => cachedPosts.value?.find((p) => p.slug === slug.value),
  },
)
</script>
...
4. The detail page uses `initialData` to render cached fields instantly.
