...
const { data: note, status } = useConvexQuery(api.notes.get, () =>
...
const {
  ...
  status,
  error,
} = await useConvexQuery(api.tasks.get, () => {
...
const { data: profile, status } = useConvexQuery(api.users.getProfile, () =>
...
<template>
  <div>
    <div>
      <button v-for="user in users" :key="user.id" @click="selectedUserId = user.id">
        {{ user.name }}
      </button>
...
const { data: selectedNote, status: detailStatus } = useConvexQuery(api.notes.get, () =>
...
<template>
  <div class="flex gap-4">
    ...
    <!-- Detail panel -->
    <div class="w-2/3">
      <p v-if="detailStatus === 'skipped'">Select a note from the list.</p>
      <p v-else-if="detailStatus === 'pending'">Loading...</p>
