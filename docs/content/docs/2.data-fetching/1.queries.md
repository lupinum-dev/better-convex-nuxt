...
4. If used in route middleware, switch to `useConvexRpc` (client) or `serverConvexQuery` (server).
::
...
For route middleware/guards, do not use query composables. Use `useConvexRpc` (client middleware) or `serverConvexQuery` (server middleware) instead.
...
3. Imperative event calls: use `useConvexRpc`.
4. Route middleware calls: use `useConvexRpc` on client navigation and `serverConvexQuery` on SSR navigation.
