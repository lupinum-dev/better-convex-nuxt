...
| `const { data } = useConvexQueryLazy(...)`                      | Returns refs immediately, resolves in background | Layouts, reusable composables, non-blocking UI           |

## Lazy Queries

`useConvexQueryLazy` is the synchronous counterpart to `useConvexQuery`. It returns refs immediately without blocking `<script setup>`, making it safe to use in **layouts**, **reusable composables**, and anywhere `await` would lose the Vue component instance context.
...
const { data: posts, pending } = useConvexQueryLazy(api.posts.list, {})
</script>
...
`useConvexQueryLazy` accepts the same arguments and options as `useConvexQuery` and returns the same `{ data, error, pending, status, refresh, clear }` shape.

::callout{icon="i-lucide-lightbulb"}
**When to use which:**
- **`useConvexQuery`** — pages and components where you want navigation to wait for data (SSR-friendly, no loading flash).
- **`useConvexQueryLazy`** — layouts, shared composables, or any context where `await` is problematic. Data arrives asynchronously; handle the `pending` state in your template.
::
...
2. Rendering reactive UI data in layouts or shared composables: use `useConvexQueryLazy`.
