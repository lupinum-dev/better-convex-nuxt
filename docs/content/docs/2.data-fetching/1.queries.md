...
Use `ref()` or `computed()` wrappers for args. Passing a plain `reactive()` object directly will not trigger re-fetches.

```ts
// WRONG: plain reactive object (no automatic re-fetch)
const filters = reactive({ categoryId: 'tech' })
const query = useConvexQuery(api.posts.byCategory, filters)

// RIGHT: computed/ref wrapper (reactive re-fetches)
const safeArgs = computed(() => ({ categoryId: filters.categoryId }))
const query2 = useConvexQuery(api.posts.byCategory, safeArgs)
```
