...
## Execution Matrix (Blocking vs Lazy)

Blocking is now explicit via `useConvexQuery` (always async), and non-blocking is explicit via `useConvexQueryLazy`.

| Pattern | What it does | Best for |
| ------- | ------------- | -------- |
| `const { data } = await useConvexQuery(...)` | Blocks navigation until data resolves | Critical above-the-fold data |
| `const { data } = useConvexQueryLazy(...)` | Starts immediately and loads in background | Secondary or progressive content |
...
const query = useConvexQuery(api.posts.byCategory, safeArgs)
```
...
4. If used in route middleware, switch to `subscribe: false` (HTTP-only one-shot mode).
::
...
By default, `await useConvexQuery()` blocks navigation until data loads. Use `useConvexQueryLazy()` to render immediately with a loading state:
...
Use `useConvexQueryLazy` for non-critical data that shouldn't block navigation:
...
const { data: related, pending } = useConvexQueryLazy(
...
- Route middleware / one-shot guards (HTTP-only environments)
...
3. Imperative event/middleware style calls: use `useConvexRpc`.
...
Skip auth token attachment for queries that intentionally run without auth:
...
| `unauthenticated`    | `boolean`          | `false`     | Skip auth checks for unauthenticated queries               |
