...
| `const { data } = useConvexQuery(..., { lazy: true })`          | Returns refs immediately, resolves in background | Layouts, reusable composables, non-blocking UI           |

## Lazy Queries

Use the `lazy: true` option for the synchronous counterpart to `useConvexQuery`. It returns refs immediately without blocking `<script setup>`, making it safe to use in **layouts**, **reusable composables**, and anywhere `await` would lose the Vue component instance context.

```vue
<script setup lang="ts">
import { api } from '~~/convex/_generated/api'

// No await — returns refs immediately
const { data: posts, pending } = useConvexQuery(api.posts.list, {}, { lazy: true })
</script>
...
The `lazy: true` option returns the same `{ data, error, pending, status, refresh, clear }` shape as the awaited version.

::callout{icon="i-lucide-lightbulb"}
**When to use which:**
- **`useConvexQuery`** (awaited) — pages and components where you want navigation to wait for data (SSR-friendly, no loading flash).
- **`useConvexQuery(..., { lazy: true })`** — layouts, shared composables, or any context where `await` is problematic. Data arrives asynchronously; handle the `pending` state in your template.
::
...
Return `null` from the args getter to skip the query:
...
// status === 'idle' when args are null
```
...
1. Rendering reactive UI data in pages/components: use `useConvexQuery`.
2. Rendering reactive UI data in layouts or shared composables: use `useConvexQuery(..., { lazy: true })`.
3. Rendering UI data without WebSocket updates: use `useConvexQuery(..., { subscribe: false })`.
4. Imperative event calls: use `useConvexCall`.
5. Route middleware calls: use `useConvexCall` on client navigation and `serverConvexQuery` on SSR navigation.
...
| `shared`           | `string`                    | `undefined` | Share one query instance across components        |
| `onData`           | `(data: T) => void`         | `undefined` | Called when new data arrives                      |
| `onError`          | `(error: Error) => void`    | `undefined` | Called when the query errors                      |
