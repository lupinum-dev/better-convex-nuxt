...
## Execution Matrix (`await` + `lazy`)

`useConvexQuery` matches Nuxt `useAsyncData` thenable semantics. Pick behavior by choosing whether you `await` and whether `lazy` is enabled:

| Pattern | What it does | Best for |
| ------- | ------------- | -------- |
| `const { data } = await useConvexQuery(..., { lazy: false })` | Blocks navigation until data resolves | Critical above-the-fold data |
| `const { data } = await useConvexQuery(..., { lazy: true })` | Non-blocking navigation while preserving thenable ergonomics | Route-level data that can load after paint |
| `const { data } = useConvexQuery(..., { lazy: true })` | Starts immediately, no top-level await required | Component-local progressive loading |
...
const { data: related, pending } = useConvexQuery(
