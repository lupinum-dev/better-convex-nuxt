...
**Client vs Server**: `useConvexQuery` is for Vue components. For server routes (`server/api/`), use [`serverConvex(event)`](/docs/server-side/server-routes) instead.
::
...
4. If used in route middleware, switch to `useConvex()` (client) or `serverConvex(event)` (server).
   ::
...
For route middleware/guards, do not use query composables. Use `useConvex()` (client middleware) or `serverConvex(event)` (server middleware) instead.
...
4. Route middleware calls: use `useConvex()` on client navigation and `serverConvex(event)` on SSR navigation.
