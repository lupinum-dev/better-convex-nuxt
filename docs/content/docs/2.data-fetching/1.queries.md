...
<template>
  <div v-if="status === 'skipped'">Select a category</div>
...
// status === 'skipped' when args are null
```
...
3. Imperative one-shot calls: use `useConvex()` to get the client, then call `.query()` / `.mutation()` / `.action()`.
...
::note
All options can be configured globally via `convex.query` in `nuxt.config.ts`. Per-query options override global defaults.
::
...
| `status`  | `ComputedRef<QueryStatus>` | `'skipped'` \| `'pending'` \| `'success'` \| `'error'`  |
