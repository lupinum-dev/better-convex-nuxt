...
The runtime-only `cause` is deliberately excluded from JSON, structured clone,
SSR HTML, and normal inspection. Direct server-side access still returns the
raw upstream value: `console.error(error.cause)` can therefore expose response
bodies, credentials, or other sensitive context. Never return it, log it
wholesale, parse its message for classification, or copy it into public fields.

`cause` is diagnostic implementation detail rather than a stable API contract.
Use reviewed public fields for product behavior and application-owned static
operation/correlation metadata for server diagnostics.
