---
title: User Augmentation
description: Add custom claims typing to useConvexAuth().user with module augmentation.
navigation:
  icon: i-lucide-user-round-cog
---

## Goal

Make `useConvexAuth().user` understand app-specific claims like `role` or `stripeCustomerId`.

## Add Module Augmentation

```ts [app/types/convex-user.d.ts]
declare module 'better-convex-nuxt' {
  interface ConvexUser {
    role?: 'admin' | 'member' | 'viewer'
    stripeCustomerId?: string
    organizationId?: string
  }
}

export {}
```

## Use It in Components

```vue
<script setup lang="ts">
const { user } = useConvexAuth()

const isAdmin = computed(() => user.value?.role === 'admin')
</script>

<template>
  <div>
    <p>Customer: {{ user?.stripeCustomerId || 'none' }}</p>
    <p v-if="isAdmin">Admin tools enabled</p>
  </div>
</template>
```

## Best Practice

- Treat `useConvexAuth().user` as UI-friendly identity + claims.
- Keep authoritative authorization in Convex query and mutation handlers. An application-owned UI capability helper may hide unavailable controls, but it is not an authorization boundary.
