...
The endpoint values are public. `BCN_AUTH_PROXY_IP_SECRET` is not: generate it separately, set the same value in Nuxt and Convex, and leave the example blank until then so missing configuration fails closed. Do not place it in `runtimeConfig.public`, commit it, or expose it to browser code.
