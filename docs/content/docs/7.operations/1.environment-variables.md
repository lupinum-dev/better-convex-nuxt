...
BCN_AUTH_PROXY_IP_SECRET=use-a-separate-random-secret
```

The endpoint values are public. `BCN_AUTH_PROXY_IP_SECRET` is not: do not place it in `runtimeConfig.public`, commit it, or expose it to browser code.
...
| `CONVEX_SITE_URL`          | Yes                | Exact Convex HTTP Actions origin used as Convex session JWT issuer       |
...
pnpm exec convex env set CONVEX_SITE_URL https://example.convex.site
...
4. Convex has its own exact `CONVEX_SITE_URL`.
