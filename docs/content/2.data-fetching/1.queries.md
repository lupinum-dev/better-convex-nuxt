...
Queries fetch data from Convex with SSR support and real-time subscriptions. Data updates automatically when any client makes changes.
...
**Client vs Server**: `useConvexQuery` is for Vue components. For server routes (`server/api/`), use [`fetchQuery`](/server-side/server-routes) instead.
::
...
By default, queries run on the server during SSR. Use `server: false` for client-only data:

```ts
// Skip SSR - useful for session-specific data
const { data } = await useConvexQuery(
  api.session.get,
  {},
  { server: false }
...
See [Core Concepts](/getting-started/concepts#loading-strategy-matrix) for all loading strategy combinations.
...
function handleClear() {
  clear() // data = undefined, status = 'idle'
}
```
...
- [Pagination](/data-fetching/pagination) - Load More and infinite scroll
- [Caching & Data Reuse](/data-fetching/caching-reuse) - Instant navigation with cached data
- [SSR & Hydration](/server-side/ssr-hydration) - Server rendering patterns
