...
try {
  ...
  if (!html.includes('MCP agent starter') || !html.includes('data-testid="auth-form"')) {
    throw new Error('Production root did not render the MCP starter auth surface')
  }

  console.log(`Production root render passed: HTTP ${response.status}, MCP starter auth surface`)
} catch (error) {
