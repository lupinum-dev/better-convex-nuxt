...
- [x] Read and classify the current starter-ladder docs:
...
- [x] Run a focused starter docs scan:
...
- [x] Classify hits as canonical starter ladder, advanced bridge material,
      stale beginner bridge/CMS wording, or false positive.
- [x] Record the baseline in this plan before editing.

Baseline: `first-live-query` already maps cleanly to `--template public` and
explicit `query.public` / `mutation.public`. `build-a-signed-in-todo-app`
already maps cleanly to `--template personal`, protected lanes, and no tenant
model. `permissions/setup` already anchors the workspace starter and avoids MCP
and bridge setup. `mcp-tools/getting-started` explained a read tool but did not
start from `--template workspace-mcp` or name the read -> bounded-write ->
operation-backed destructive progression. The examples pages listed
`07-mcp-reference` and `08-component-mini-cms` as advanced entries, but the
labels did not clearly separate the workspace-MCP branch from package-integration
bridge material.
...
- [x] Ensure the guide says: use `--template public` when you want the generated
      path, then keep the explicit `query.public` / `mutation.public` shape.
- [x] Keep the hand-built walkthrough only as a learning path, not as a second
      starter source of truth.
- [x] Verify no auth/workspace/MCP concepts leak into the public starter path.

No edit was needed; the page already matched the public starter path.
...
- [x] Ensure the guide says: `personal` is signed-in user scope without tenant
      isolation or workspace roles.
- [x] Ensure protected lanes use `query.protected` / `mutation.protected`.
- [x] Keep workspace escalation as a next step, not mixed into the personal
      starter.

No edit was needed; the page already matched the personal starter path.

### 4. Workspace Starter Guide

- [x] Update `apps/docs/content/docs/08.permissions/1.setup.md` only where it
      teaches workspace setup without anchoring to the `workspace` starter.
- [x] Ensure the guide says: `workspace` introduces actor resolution, tenant
      isolation, roles/guards, and permission projection.
- [x] Ensure the principal -> actor -> guard/load/authorize -> handler model is
      preserved.
- [x] Avoid copying MCP or bridge setup into this guide.

No edit was needed; the page already matched the workspace starter path.

### 5. Workspace MCP Starter Guide

- [x] Update `apps/docs/content/docs/14.mcp-tools/1.getting-started.md` only
      where it does not clearly map to the `workspace-mcp` starter.
- [x] Ensure the guide says: `workspace-mcp` starts from the workspace baseline
      plus MCP projection.
- [x] Ensure the first MCP path is a read tool, then bounded writes, then
      operation-backed destructive tools.
- [x] Ensure destructive MCP references `tool.operation(...)` only.
- [x] Ensure signed forwarding is not described as authorization.

### 6. Remove Beginner Bridge Leakage If Found

- [x] Check first-reader starter paths for package-author bridge concepts.
- [x] Keep `component-bridge` docs linked only as advanced/package-author
      material.
- [x] If starter pages currently send beginners to bridge setup, replace that
      path with Ginko-owned or advanced package-integration wording.
- [x] Do not delete the advanced bridge docs in this sprint unless they are
      linked from the starter ladder as beginner setup.

No starter page sends beginners to bridge setup. The examples pages now label
`08-component-mini-cms` as advanced package-integration material rather than a
normal starter path.
...
- [x] Focused starter docs scan from step 1 returns only intentional starter
...
- [x] Add a Sprint 72 completion note to
      `meta/trellis-1.0-refactor-plan.md`.
- [x] Mark Public/Personal/Workspace/Workspace MCP starter guide items complete
      only for paths actually edited or proven current.
- [x] Mark "Delete beginner bridge references" complete only if the focused scan
      proves bridge references are advanced-only.
- [x] Leave backend builder, operations, MCP projection, trusted forwarding,
