...
- [x] Run `pnpm run check` in Ginko CMS.
- [x] Classify each failure as formatting, lint guardrail, typecheck,
      publish-specifier, unit/integration test, or docs/setup wording.
- [x] Record the first failing command and representative error in this plan.

Baseline: the first full check failed in `format:check`. After formatting, lint
exposed one formatter/lint conflict in IPv6 mask notation. The next full check
reached tests and exposed four real failures: missing exact signed-forwarding
metadata on the asset reference rebuild mutation, a public-list assertion on a
data-only collection, unsupported-locale behavior that now fails loudly, and the
package-consumer fixture trying to resolve unpublished `@lupinum/trellis` from
the registry.

### 2. Fix Only Real Gate Failures

- [x] Fix formatting failures with the formatter, not manual churn.
- [x] Fix lint/guardrail failures at the source of truth, not by weakening the
      guardrail.
- [x] Fix tests by preserving the new Trellis 1.0 shape:
      `@lupinum/trellis-bridge`, signed `_trellisForwarding`, explicit
      `functionRefModule`, and `mcp.tool.operation(...)`.
- [x] If docs/setup wording fails a targeted check, update docs so users see
      `ginko-cms init`, Ginko-owned bridge health commands, and Ginko
      terminology.

The targeted scan found no current-user-facing docs/setup hits for deleted
Trellis APIs. Remaining hits are package-boundary/no-zombie-path tests that
assert the old paths are absent.

### 3. Add A Narrow Guardrail Only If A Zombie Path Is Found

- [x] If the check exposes old public paths that are not currently guarded,
      add one focused scan/test for that exact path.
- [x] Do not add broad regex checks over historical `docs/refactor/**` notes.
- [x] Keep historical notes historical; current docs and setup output must not
      teach deleted Trellis APIs.

No new zombie path was found, so no new guardrail was added.

### 4. Verify Ginko

- [x] `pnpm run check`
- [x] `rg -n "tool\\.fromOperation|_trustedForwardingKey|_trustedForwarding\\b|@lupinum/trellis/(functions|bridge)" README.md docs packages test -g '!docs/refactor/**'`
      returns no current-user-facing deleted-path hits, or every remaining hit
      is deliberately documented in this plan.
- [x] `git diff --check`
- [x] `git status --short` shows only intentional sprint changes before commit.

Remaining scan hits are intentional test references in
`test/shared/mcp-tools.test.ts` and `test/module/package-boundaries.test.ts`.

### 5. Verify Trellis If Touched

- [x] `pnpm run check:repo-policies`
- [x] `pnpm exec oxfmt --check meta/refactor/sprint70-ginko-full-check-gate-plan.md meta/trellis-1.0-refactor-plan.md`
- [x] `git diff --check`

### 6. Update The Refactor Tracker

- [x] Add a Sprint 70 completion note to
      `meta/trellis-1.0-refactor-plan.md`.
- [x] Mark Ginko full local check proof complete only if `pnpm run check`
      passes.
- [x] Mark Ginko docs/setup wording complete only if current docs/setup scans
      prove old Trellis API wording is gone.
- [x] Leave packed Trellis package install validation open unless packed
