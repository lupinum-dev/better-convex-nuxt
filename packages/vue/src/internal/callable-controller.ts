...
export function createCallableController<Args, Result>(
    ...
    try {
      ...
      if (committed) {
        if (handlers.onSuccess)
          runCallback(() => handlers.onSuccess!(result, args));
      ...
      if (committed) {
        if (handlers.onError)
          runCallback(() => handlers.onError!(normalized, args));
  ...
  stopIdentity =
    input.subscribeIdentityChange?.(onIdentityMaybeChanged) ?? null;
