 ...
 *
 * Authentication settlement happens before the call captures its identity
 * generation and before `invoke`. A settlement-time identity transition may
 * mask the provisional pending state, so the latest still-live attempt restores
 * pending immediately before dispatch. Reset and newer attempts remain final.
 */
export function createCallableController<Args, Result>(
  ...
  const run = async (args: Args): Promise<Result> => {
    ...
    try {
      ...
      // Settlement defines the identity under which the call is dispatched.
      // If that transition masked the provisional state, only the latest live
      // attempt may restore it. A reset/newer call increments attemptRevision.
      ...
      const stale =
        isIdentityChangedError(normalized) ||
        (generation !== null && getIdentityGeneration() !== generation)
