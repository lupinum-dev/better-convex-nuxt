...
describe('public todo example', () => {
  it('creates, toggles, lists, and removes todos', async () => {
    ...
    const firstId = await ctx.raw.mutation(api.domain.todos.create, {
      title: 'Ship public demo',
    })
    const secondId = await ctx.raw.mutation(api.domain.todos.create, {
      title: 'Write docs',
    })

    let todos: Todo[] = await ctx.raw.query(api.domain.todos.list, {})
    expect(todos).toHaveLength(2)
    expect(todos[0]?.title).toBe('Write docs')
    expect(todos[1]?.title).toBe('Ship public demo')

    await ctx.raw.mutation(api.domain.todos.toggle, { id: firstId })

    todos = await ctx.raw.query(api.domain.todos.list, {})
    expect(todos.find((todo) => todo._id === firstId)?.completed).toBe(true)

    await ctx.raw.mutation(api.domain.todos.remove, { id: secondId })
