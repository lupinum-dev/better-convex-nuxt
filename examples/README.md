...
| `03-team-workspace`     | `CONVEX_URL`, `CONVEX_SITE_URL` | `SITE_URL` for Better Auth callback origin, `BETTER_AUTH_SECRET` for auth signing, `CONVEX_TRUSTED_FORWARDING_KEY` for trusted server-to-Convex calls, `TRELLIS_MCP_CONFIRMATION_KEY` for destructive MCP confirmation signing, `TEAM_WORKSPACE_WEBHOOK_SECRET` for the webhook route boundary, `TEAM_WORKSPACE_WEBHOOK_AUTH_ID` for the delegated workspace user used by the webhook example                                                          |
| `04-saas-platform`      | `CONVEX_URL`, `CONVEX_SITE_URL` | `SITE_URL` for Better Auth callback origin, `BETTER_AUTH_SECRET` for auth signing, `CONVEX_TRUSTED_FORWARDING_KEY` for trusted server-to-Convex calls, `PROJECT_BOARD_WEBHOOK_SECRET` for the webhook route boundary                                                                                                                                                                                                                                   |
...
| `07-mcp-reference`      | `CONVEX_URL`, `CONVEX_SITE_URL` | `SITE_URL` for Better Auth callback origin, `BETTER_AUTH_SECRET` for auth signing, `CONVEX_TRUSTED_FORWARDING_KEY` for trusted server-to-Convex calls, `TRELLIS_MCP_CONFIRMATION_KEY` for destructive MCP confirmation signing, `MCP_RATE_LIMIT_REDIS_URL` for the distributed MCP rate-limit store, `MCP_REFERENCE_WEBHOOK_SECRET` for the webhook route boundary, `MCP_REFERENCE_WEBHOOK_AUTH_ID` for the delegated user used by the webhook example |
