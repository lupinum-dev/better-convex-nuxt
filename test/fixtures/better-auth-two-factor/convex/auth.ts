...
import { createTwoFactorAuthPlugins } from './betterAuth/schemaPlugins'
...
export async function createAuth(ctx: AuthCtx<DataModel>) {
  try {
    ...
    const auth = betterAuth({
      ...
      plugins: [
        ...createTwoFactorAuthPlugins(authIssuer),
        convexAuth({
      ...
      // This fixture deliberately enables Better Auth's signed session cache.
      // The /convex/token tests then alter the canonical row and prove that
      // BCN re-reads persisted state instead of trusting cached middleware.
      session: {
