...
describe('useConvexAuth() under an auth-disabled build (vNext §5.3)', () => {
  it('returns the exact stable disabled contract', async () => {
    ...
    await expect(result.signOut()).resolves.toBe(null)
    await expect(result.refreshAuth()).resolves.toBeUndefined()
    await expect(result.awaitAuthReady()).resolves.toBe(false)
  })
})
