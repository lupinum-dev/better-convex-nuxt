...
describe("package artifact coordinates", () => {
  it.each([
    ...
    expect(() =>
      assertReleaseEligiblePackageVersion(packageName, version),
    ).toThrow("retired unpublished source identity");
  });

  it("does not generalize the retirement guard to successor or unrelated versions", () => {
    expect(
      assertReleaseEligiblePackageVersion("better-convex-nuxt", "0.8.0-beta.7"),
    ).toBe("0.8.0-beta.7");
    expect(
      assertReleaseEligiblePackageVersion("@better-convex/mcp", "0.1.0-beta.1"),
    ).toBe("0.1.0-beta.1");
    expect(
      assertReleaseEligiblePackageVersion("unrelated-package", "0.8.0-beta.6"),
    ).toBe("0.8.0-beta.6");
  });
  ...
  it("resolves the root Nuxt package to one qualified immutable directory", () => {
    ...
    expect(coordinates.relativePaths).toEqual({
      ...
      tarball:
        ".release-artifacts/nuxt/0.7.0-beta.1/better-convex-nuxt-0.7.0-beta.1.tgz",
    });
  ...
  it.each([
    ...
    "1.0.0-alpha-1",
    "1.0.0-0A",
  ])("accepts canonical package version %j", (version) => {
  ...
  it.each([
    ...
    expect(() => validatePackageArtifactVersion(version)).toThrow(
  ...
  it("rejects a version that cannot fit a filesystem component or tarball basename", () => {
    expect(() =>
      validatePackageArtifactVersion(`1.0.0-${"a".repeat(256)}`),
  ...
  it("derives npm canonical tarball basenames without using them as package directories", () => {
    expect(
      canonicalNpmTarballFilename("@better-convex/mcp", "1.2.3-beta.4"),
    ).toBe("better-convex-mcp-1.2.3-beta.4.tgz");
    ...
    expect(
      join("mcp", canonicalNpmTarballFilename("@scope/name", "1.0.0")),
    ).not.toBe(
  ...
  it("preserves sibling and legacy artifacts and rejects an existing immutable target", () => {
    ...
    expect(
      assertPackageArtifactWriteTarget("nuxt", { repositoryRoot: root }),
    ).toEqual(coordinates);
  ...
  it.each(["artifact-root", "package-root"])(
    "rejects a %s symlink alias",
    (location) => {
      const root = createRepository();
