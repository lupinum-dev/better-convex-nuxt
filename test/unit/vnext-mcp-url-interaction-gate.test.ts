...
async function connectClient(
  ...
  const transport = new StreamableHTTPClientTransport(
    new URL("https://mcp-lab.invalid/mcp"),
    {
  ...
  const client = new Client(
    {
      name: supportsUrlElicitation
        ? "url-capable-client"
        : "url-incapable-client",
      version: "0.0.0",
    },
    {
...
describe("vNext published URL-interaction capability gate", () => {
  it("proves 2025 stateless serving cannot distinguish capable and incapable clients", async () => {
    ...
    try {
      ...
      const capableToolCall = capableRequests.find(
        (request) => request.method === "tools/call",
      );
      const incapableToolCall = incapableRequests.find(
        (request) => request.method === "tools/call",
      );
      expect(capableToolCall).toEqual(incapableToolCall);
      expect(factoryContexts).toHaveLength(
        capableRequests.length + incapableRequests.length,
      );
      expect(factoryContexts.every((context) => context.era === "legacy")).toBe(
        true,
      );
      expect(
        factoryContexts.every((context) => !("clientCapabilities" in context)),
      ).toBe(true);
    } finally {
      await Promise.allSettled([
        capable.client.close(),
        incapable.client.close(),
      ]);
      await handler.close();
    }
  });
