...
describe('package artifact coordinate CLI', () => {
  it('prints only descriptor-derived GitHub output values', () => {
    ...
    expect(result.stdout.trim().split('\n')).toEqual([
      ...
      'directory=.release-artifacts/nuxt/0.8.0-beta.14',
      'evidence=.release-artifacts/nuxt/0.8.0-beta.14/artifact.json',
      'package_id=nuxt',
      'package_name=better-convex-nuxt',
      'tarball=.release-artifacts/nuxt/0.8.0-beta.14/better-convex-nuxt-0.8.0-beta.14.tgz',
      'tarball_filename=better-convex-nuxt-0.8.0-beta.14.tgz',
  ...
  it('prints Vue artifact coordinates from the reviewed nested package', () => {
    ...
    expect(result.stdout.trim().split('\n')).toEqual([
      ...
      'directory=.release-artifacts/vue/0.8.0-beta.14',
      'evidence=.release-artifacts/vue/0.8.0-beta.14/artifact.json',
      'package_id=vue',
      'package_name=better-convex-vue',
      'tarball=.release-artifacts/vue/0.8.0-beta.14/better-convex-vue-0.8.0-beta.14.tgz',
      'tarball_filename=better-convex-vue-0.8.0-beta.14.tgz',
      'version=0.8.0-beta.14',
    ])
  })
