...
describe("Convex-native official MCP handler composition", () => {
  it("serves explicit read and write tools while keeping bearer data outside application context", async () => {
      ...
      createServer(context, access) {
        ...
        server.registerTool(
          ...
          ({ query }, extra) => {
            ...
            context.operations.push(
              `search:${access.issuer}:${access.subject}`,
            );
            const output = {
        ...
        server.registerResource(
          ...
          async (uri, { id }) => {
            ...
            return {
              contents: [
                { uri: uri.href, mimeType: "text/plain", text: title },
              ],
            };
          },
        );
    ...
    try {
      ...
      ).toEqual(["rename_note", "search_notes"]);
      ...
      expect(search.content).toEqual([
        { type: "text", text: "1 note matched: Alpha." },
      ]);
      ...
      expect(rename.content).toEqual([
        { type: "text", text: "Renamed note-1 to “Beta”." },
      ]);
      ...
      await expect(
        client.readResource({ uri: "note://note-1" }),
      ).resolves.toMatchObject({
        contents: [
          { uri: "note://note-1", mimeType: "text/plain", text: "Beta" },
        ],
      });
  ...
  it("serves fixed RFC 9728 metadata and binds every challenge to its exact URL", async () => {
    ...
    const protectedResponse = await handler.fetch(
      {},
    ...
    const denied = await handler.fetch(
      {},
      new Request(
        "https://attacker-selected.invalid/mcp?resource=https://attacker.invalid",
        {
          method: "POST",
          headers: {
  ...
  it("composes tool quotas from verified identity and host-trusted context without reading IP headers", async () => {
    ...
    expect(
      (await call("alice-client-1", "edge-a", "search_notes")).isError,
    ).not.toBe(true);
    expect(
      (await call("alice-client-1", "edge-a", "search_notes", "198.51.100.99"))
        .isError,
    ).toBe(true);
    expect(
      (await call("alice-client-1", "edge-a", "rename_note")).isError,
    ).not.toBe(true);
    expect(
      (await call("alice-client-2", "edge-a", "search_notes")).isError,
    ).not.toBe(true);
    expect(
      (await call("bob-client-1", "edge-a", "search_notes")).isError,
    ).not.toBe(true);
    expect(
      (await call("alice-client-1", "edge-b", "search_notes")).isError,
    ).not.toBe(true);
    ...
    expect(concurrent.filter((result) => result.isError === true)).toHaveLength(
      1,
    );
  });
});
