...
describe("MCP explicit Convex operation mapping", () => {
  it("binds each tool to one fixed operation kind and generated reference", async () => {
      ...
      createServer(context, access) {
        ...
        server.registerTool(
          ...
          async (input) => {
            ...
            return {
              content: [
                { type: "text", text: `${output.titles.length} note matched.` },
              ],
    ...
    const transport = new StreamableHTTPClientTransport(resource, {
      ...
      fetch: (input, init) =>
        handler.fetch(application, new Request(input, init)),
    });
    ...
    try {
      ...
      await expect(
        client.callTool({ name: "notes:rename", arguments: {} }),
      ).rejects.toThrow("Tool notes:rename not found");
      expect(calls).toHaveLength(1);
